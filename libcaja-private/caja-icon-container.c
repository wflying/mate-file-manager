                   ...
                   double x, double y)
{
    ...
    g_warning("AJ_DEBUG enter icon_set_position icon->x=%f icon->y = %f",icon->x,icon->y);
    ...
    {
	g_warning("AJ_DEBUG enter fixed_size\n");
	...
	g_warning("AJ_DEBUG icon_set_position x=%f min_x=%f max_x=%f\n",x,min_x,max_x);
	...
	g_warning("AJ_DEBUG icon_set_position y=%f min_y=%f max_y=%f\n",y,min_y,max_y);
    ...
    {
	g_warning("AJ_DEBUG　enter ICON_UNPOSITIONED_VALUE\n");
        icon->x = 0;
    }
    ...
    g_warning("AJ_DEBUG atend icon_set_position iconx = %f icony = %f\n",icon->x,icon->y);
    g_warning("AJ_DEBUG atend icon_set_position x = %f y = %f\n",x,y);
               ...
               gboolean update_position)
{
    ...
    g_warning("AJ_DEBUG enter icon_set_size x=%d,y=%d\n",icon->x, icon->y);
                      ...
                      CajaIcon *icon)
{
    ...
    if (icon == container->details->stretch_icon)
    {
        ...
        {
	    g_warning("AJ_DEBUG enter icon_toggle_selected icon->x = %d,icon->y=%d\n",icon->x, icon->y);
...
caja_icon_container_set_rtl_positions (CajaIconContainer *container)
{
	...
	g_warning("AJ_DEBUG enter caja_icon_container_set_rtl_positions x=%f y=%f\n",x,icon->y);
...
reload_icon_positions (CajaIconContainer *container)
{
    ...
    for (p = container->details->icons; p != NULL; p = p->next)
    {
        ...
        {
	    g_warning("AJ_DEBUG enter reload_icon_positions x=%d y=%d\n",position.x,position.y);
                               ...
                               gboolean update_position)
{
    ...
    g_warning("AJ_DEBUG caja_icon_container_move_icon x=%d y= %d\n",x,y);
    g_warning("AJ_DEBUG caja_icon_container_move_icon iconx=%f icony= %f\n",icon->x,icon->y);
    ...
    if (!details->auto_layout)
    {
        ...
        {
            icon_set_position (icon, x, y);
    	    g_warning("AJ_DEBUG after aaaaaaaa caja_icon_container_move_icon iconx=%f icony= %f\n",icon->x,icon->y);
    ...
    if (emit_signal)
    {
	g_warning("AJ_DEBUG enter caja_icon_container_move_icon icon->saved_ltr_x=%f,icon->y=%f\n",icon->saved_ltr_x,icon->y);
...
update_stretch_at_idle (CajaIconContainer *container)
{
    ...
    g_warning("AJ_DEBUG enter update_stretch_at_idle x=%d y=%d\n",world_x,world_y);
...
undo_stretching (CajaIconContainer *container)
{
    ...
    g_warning("AJ_DEBUG enter undo_stretching x=%f y=%f",container->details->stretch_initial_x,container->details->stretch_initial_y);
                      ...
                      CajaIcon *icon)
{
    ...
    g_warning("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    g_warning("AJ_DEBUG enter assign_icon_position position.x=%f position.y=%f",position.x,position.y);
    if (!container->details->auto_layout)
    {
        if (have_stored_position)
        {
	    g_warning("AJ_DEBUG　enter assign_icon_position have_stored_position");
            icon_set_position (icon, position.x, position.y);
            icon->saved_ltr_x = icon->x;
        }
        else
...
caja_icon_container_unstretch (CajaIconContainer *container)
{
    ...
    for (p = container->details->icons; p != NULL; p = p->next)
    {
        ...
        if (icon->is_selected)
        {
	    g_warning("AJ_DEBUG enter caja_icon_container_unstretch x=%d,y=%d\n",icon->x, icon->y);
            caja_icon_container_move_icon (container, icon,
                                           icon->x, icon->y,
                                           1.0,
                                           FALSE, TRUE, TRUE);
...
void
caja_icon_container_set_keep_aligned_force (CajaIconContainer *container,
...
caja_icon_container_freeze_icon_positions (CajaIconContainer *container)
{
    ...
    for (p = container->details->icons; p != NULL; p = p->next)
    {
        ...
        position.y = icon->y;
        position.scale = icon->scale;
	g_warning("AJ_DEBUG enter caja_icon_container_freeze_icon_positions x=%d y=%d\n",position.x,position.y);
        g_signal_emit (container, signals[ICON_POSITION_CHANGED], 0,
                       icon->data, &position);
    }
